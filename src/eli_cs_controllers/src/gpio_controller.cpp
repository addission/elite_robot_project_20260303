...
void GPIOController::publishIO() {
    for (size_t i = 0; i < STANDARD_DIG_GPIO_NUM; ++i) {
        io_msg_.standard_out[i] = static_cast<bool>(state_interfaces_[i].get_optional().value_or(0.0));
        io_msg_.standard_in[i] = static_cast<bool>(state_interfaces_[i + (int)StateOffset::STANDARD_DIG_IN].get_optional().value_or(0.0));
    }
    for (size_t i = 0; i < CONF_DIG_GPIO_NUM; ++i) {
        io_msg_.config_out[i] = static_cast<bool>(state_interfaces_[i + (int)StateOffset::CONFIGURE_DIG_OUT].get_optional().value_or(0.0));
        io_msg_.config_in[i] = static_cast<bool>(state_interfaces_[i + (int)StateOffset::CONFIGURE_DIG_IN].get_optional().value_or(0.0));
    }
    for (size_t i = 0; i < TOOL_DIG_GPIO_NUM; ++i) {
        io_msg_.tool_out[i] = static_cast<bool>(state_interfaces_[i + (int)StateOffset::TOOL_DIG_OUT].get_optional().value_or(0.0));
        io_msg_.tool_in[i] = static_cast<bool>(state_interfaces_[i + (int)StateOffset::TOOL_DIG_IN].get_optional().value_or(0.0));
    }

    io_msg_.standard_analog_in[0].type = (uint8_t)state_interfaces_[(int)StateOffset::STANDARD_ANALOG_IN_TYPE1].get_optional().value_or(0.0);
    io_msg_.standard_analog_in[0].value = state_interfaces_[(int)StateOffset::STANDARD_ANALOG_IN1].get_optional().value_or(0.0);

    io_msg_.standard_analog_in[1].type = (uint8_t)state_interfaces_[(int)StateOffset::STANDARD_ANALOG_IN_TYPE2].get_optional().value_or(0.0);
    io_msg_.standard_analog_in[1].value = state_interfaces_[(int)StateOffset::STANDARD_ANALOG_IN2].get_optional().value_or(0.0);

    io_msg_.standard_analog_out[0].type = state_interfaces_[(int)StateOffset::STANDARD_ANALOG_OUT_TYPE1].get_optional().value_or(0.0);
    io_msg_.standard_analog_out[0].value = state_interfaces_[(int)StateOffset::STANDARD_ANALOG_OUT1].get_optional().value_or(0.0);

    io_msg_.standard_analog_out[1].type = state_interfaces_[(int)StateOffset::STANDARD_ANALOG_OUT_TYPE2].get_optional().value_or(0.0);
    io_msg_.standard_analog_out[1].value = state_interfaces_[(int)StateOffset::STANDARD_ANALOG_OUT2].get_optional().value_or(0.0);

    io_pub_->publish(io_msg_);
}

void GPIOController::publishToolData() {
    tool_data_msg_.mode = static_cast<uint8_t>(state_interfaces_[(int)StateOffset::TOOL_MODE].get_optional().value_or(0.0));
    tool_data_msg_.output_voltage = state_interfaces_[(int)StateOffset::TOOL_OUTPUT_VOLTAGE].get_optional().value_or(0.0);
    tool_data_msg_.output_current = state_interfaces_[(int)StateOffset::TOOL_OUTPUT_CURRENT].get_optional().value_or(0.0);
    tool_data_msg_.temperature = state_interfaces_[(int)StateOffset::TOOL_TEMPERATURE].get_optional().value_or(0.0);
    tool_data_msg_.analog_input_type = (uint8_t)state_interfaces_[(int)StateOffset::TOOL_ANALOG_INPUT_TYPE].get_optional().value_or(0.0);
    tool_data_msg_.analog_input = state_interfaces_[(int)StateOffset::TOOL_ANALOG_INPUT].get_optional().value_or(0.0);
    tool_data_msg_.analog_output_type = (uint8_t)state_interfaces_[(int)StateOffset::TOOL_ANALOG_OUTPUT_TYPE].get_optional().value_or(0.0);
    tool_data_msg_.analog_output = state_interfaces_[(int)StateOffset::TOOL_ANALOG_OUTPUT].get_optional().value_or(0.0);
    tool_data_pub_->publish(tool_data_msg_);
}

void GPIOController::publishRobotMode() {
    auto robot_mode = static_cast<int8_t>(state_interfaces_[(int)StateOffset::ROBOT_MODE].get_optional().value_or(0.0));
...
void GPIOController::publishSafetyMode() {
    auto safety_mode = static_cast<uint8_t>(state_interfaces_[(int)StateOffset::SAFETY_MODE].get_optional().value_or(0.0));
...
void GPIOController::publishTaskRunning() {
    auto task_running_value = static_cast<uint8_t>(state_interfaces_[(int)StateOffset::TASK_RUNNING].get_optional().value_or(0.0));
...
controller_interface::CallbackReturn ELITE_CS_CONTROLLER::GPIOController::on_activate(
    const rclcpp_lifecycle::State& /*previous_state*/) {
    while (state_interfaces_[(int)StateOffset::INITIALIZED_FLAG].get_optional().value_or(0.0) == 0.0) {
...
bool GPIOController::setIO(eli_common_interface::srv::SetIO::Request::SharedPtr req, eli_common_interface::srv::SetIO::Response::SharedPtr resp) {
    if (req->fun == req->FUN_SET_DIGITAL_OUT && req->pin >= 0 && req->pin <= STANDARD_DIG_GPIO_NUM) {
        // io async success
        (void)command_interfaces_[(int)CommandOffset::IO_ASYNC_SUCCESS].set_value(ASYNC_WAITING);
        (void)command_interfaces_[req->pin].set_value(static_cast<double>(req->state));

        RCLCPP_INFO(get_node()->get_logger(), "Setting standard digital output '%d' to state: '%1.0f'.", req->pin, req->state);

        if (!waitForAsyncCommand([&]() { return command_interfaces_[(int)CommandOffset::IO_ASYNC_SUCCESS].get_optional().value_or(0.0); })) {
            RCLCPP_WARN(get_node()->get_logger(),
                        "Could not verify that io was set. (This might happen when using the "
                        "mocked interface)");
        }

        resp->success = static_cast<bool>(command_interfaces_[(int)CommandOffset::IO_ASYNC_SUCCESS].get_optional().value_or(0.0));
        return resp->success;
    } else if(req->fun == req->FUN_SET_CONFIGURE_OUT && req->pin >= 0 && req->pin <= CONF_DIG_GPIO_NUM) {
        // io async success
        (void)command_interfaces_[(int)CommandOffset::IO_ASYNC_SUCCESS].set_value(ASYNC_WAITING);
        (void)command_interfaces_[req->pin + (int)CommandOffset::CONFIGURE_DIG_OUT].set_value(static_cast<double>(req->state));
        ...
        // io async success
        (void)command_interfaces_[(int)CommandOffset::IO_ASYNC_SUCCESS].set_value(ASYNC_WAITING);
        (void)command_interfaces_[req->pin + (int)CommandOffset::TOOL_DIG_OUT].set_value(static_cast<double>(req->state));
        ...
        (void)command_interfaces_[(int)CommandOffset::IO_ASYNC_SUCCESS].set_value(ASYNC_WAITING);
        if(req->pin == 0) {
            (void)command_interfaces_[(int)CommandOffset::STANDARD_ANALOG_OUT].set_value(static_cast<double>(req->state));
            (void)command_interfaces_[(int)CommandOffset::STANDARD_ANALOG_OUT + 1].set_value(static_cast<double>(req->analog_type));
        } else {
            (void)command_interfaces_[(int)CommandOffset::STANDARD_ANALOG_OUT + 2].set_value(static_cast<double>(req->state));
            (void)command_interfaces_[(int)CommandOffset::STANDARD_ANALOG_OUT + 3].set_value(static_cast<double>(req->analog_type));
        }
        ...
        (void)command_interfaces_[(int)CommandOffset::IO_ASYNC_SUCCESS].set_value(ASYNC_WAITING);
        (void)command_interfaces_[(int)CommandOffset::TOOL_VOLTAGE].set_value(static_cast<double>(req->state));
...
bool GPIOController::setSpeedSlider(eli_common_interface::srv::SetSpeedSliderFraction::Request::SharedPtr req,
    ...
    if (req->speed_slider_fraction >= 0.01 && req->speed_slider_fraction <= 1.0) {
        ...
        // reset success flag
        (void)command_interfaces_[(int)CommandOffset::TARGET_SPEED_FRACTION_SUCCESS].set_value(ASYNC_WAITING);
        // set commanding value for speed slider
        (void)command_interfaces_[(int)CommandOffset::TARGET_SPEED_FRACTION].set_value(
            static_cast<double>(req->speed_slider_fraction));

        if (!waitForAsyncCommand(
                [&]() { return command_interfaces_[(int)CommandOffset::TARGET_SPEED_FRACTION_SUCCESS].get_optional().value_or(0.0); })) {
            RCLCPP_WARN(get_node()->get_logger(),
                        "Could not verify that target speed fraction was set. (This might happen "
                        "when using the mocked interface)");
        }
        resp->success = static_cast<bool>(command_interfaces_[(int)CommandOffset::TARGET_SPEED_FRACTION_SUCCESS].get_optional().value_or(0.0));
    } else {
        RCLCPP_WARN(get_node()->get_logger(),
...
bool GPIOController::handBackControl(std_srvs::srv::Trigger::Request::SharedPtr /*req*/,
                                     std_srvs::srv::Trigger::Response::SharedPtr resp) {
    // reset success flag
    (void)command_interfaces_[(int)CommandOffset::HAND_BACK_CONTROL_SUCCESS].set_value(ASYNC_WAITING);
    // call the service in the hardware
    (void)command_interfaces_[(int)CommandOffset::HAND_BACK_CONTROL].set_value(1.0);

    if (!waitForAsyncCommand(
            [&]() { return command_interfaces_[(int)CommandOffset::HAND_BACK_CONTROL_SUCCESS].get_optional().value_or(0.0); })) {
        RCLCPP_WARN(get_node()->get_logger(),
                    "Could not verify that hand_back_control was correctly triggered. (This "
                    "might happen when using the mocked interface)");
    }
    resp->success = static_cast<bool>(command_interfaces_[(int)CommandOffset::HAND_BACK_CONTROL_SUCCESS].get_optional().value_or(0.0));
...
bool GPIOController::setPayload(const eli_common_interface::srv::SetPayload::Request::SharedPtr req,
    ...
    // reset success flag
    (void)command_interfaces_[(int)CommandOffset::PAYLOAD_SUCCESS].set_value(ASYNC_WAITING);

    (void)command_interfaces_[(int)CommandOffset::MASS].set_value(static_cast<double>(req->mass));
    (void)command_interfaces_[(int)CommandOffset::COG_X].set_value(req->center_of_gravity.x);
    (void)command_interfaces_[(int)CommandOffset::COG_Y].set_value(req->center_of_gravity.y);
    (void)command_interfaces_[(int)CommandOffset::COG_Z].set_value(req->center_of_gravity.z);

    if (!waitForAsyncCommand([&]() { return command_interfaces_[(int)CommandOffset::PAYLOAD_SUCCESS].get_optional().value_or(0.0); })) {
        RCLCPP_WARN(get_node()->get_logger(),
                    "Could not verify that payload was set. (This might happen when using the "
                    "mocked interface)");
    }

    resp->success = static_cast<bool>(command_interfaces_[(int)CommandOffset::PAYLOAD_SUCCESS].get_optional().value_or(0.0));
...
bool GPIOController::zeroFTSensor(std_srvs::srv::Trigger::Request::SharedPtr /*req*/,
                                  std_srvs::srv::Trigger::Response::SharedPtr resp) {
    // reset success flag
    (void)command_interfaces_[(int)CommandOffset::ZERO_FTSENSOR_SUCCESS].set_value(ASYNC_WAITING);
    // call the service in the hardware
    (void)command_interfaces_[(int)CommandOffset::ZERO_FTSENSOR].set_value(1.0);

    if (!waitForAsyncCommand([&]() { return command_interfaces_[(int)CommandOffset::ZERO_FTSENSOR_SUCCESS].get_optional().value_or(0.0); })) {
        RCLCPP_WARN(get_node()->get_logger(),
                    "Could not verify that FTS was zeroed. (This might happen when using the "
                    "mocked interface)");
    }

    resp->success = static_cast<bool>(command_interfaces_[(int)CommandOffset::ZERO_FTSENSOR_SUCCESS].get_optional().value_or(0.0));
