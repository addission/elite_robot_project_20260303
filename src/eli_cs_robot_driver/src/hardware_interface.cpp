...
void EliteCSPositionHardwareInterface::extractToolPose() {
    // 直接使用tf2四元数工具，将欧拉角转换为四元数
    tf2::Quaternion quat;
    // 若SDK未特别声明“欧拉角顺序”，通常为setRPY(roll, pitch, yaw)
    quat.setRPY(tcp_pose_[3], tcp_pose_[4], tcp_pose_[5]);
    ...
    tcp_rotation_quat_buffer_.set(quat);

    tcp_transform_.transform.rotation = tf2::toMsg(quat);
}
